...
def not_valid_dir(direc):
    ...
    if not os.path.isdir(direc):
        errors = True
        print("%s%s: Error: The input pcap directory \"%s\" does not exist.%s" % (RED, path, direc, END),
              file=sys.stderr)
    else:
        if not os.access(direc, os.R_OK):
            errors = True
            print("%s%s: Error: The \"%s\" directory does not have read permission.%s" % (RED, path, direc, END),
                  file=sys.stderr)
        if not os.access(direc, os.X_OK):
            errors = True
            print("%s%s: Error: The \"%s\" directory does not have execute permission.%s" % (RED, path, direc, END),
                  file=sys.stderr)
    ...
    if software_location == "":
        ...
        if not_valid_dir(os.path.dirname(os.path.dirname(software_location))):
            ...
            if not os.path.isfile(software_location):
                errors = True
                print("%s%s: Error: The script \"%s\" is missing.%s" % (RED, path, software_location, END),
                      file=sys.stderr)
            elif not os.access(software_location, os.R_OK):
                errors = True
                print("%s%s: Error: The script \"%s\" does not have read permission.%s" % (
                    RED, path, software_location, END))

    if not args.out_csv.endswith(".csv"):
        errors = True
        print("%s%s: Error: The output file should be a .csv file. Received \"%s\"%s" % (RED, path, args.out_csv, END),
              file=sys.stderr)
